...
Version 0.7.0 is implemented and running. The menubar shows today's running balance, and the dropdown is the Byte Flow deck: a chart-led live dashboard that adapts to the system appearance, opens fully formed with warm live rates, carries a LIVE toggle, and now offers per-channel chart windows from 30 minutes to 24 hours. COGNITION reads three AI sources — Claude Code, Codex CLI, and Gemini CLI — with an honest "N of M sources reporting" disclosure, and an ALSO ON THE BOOKS strip carries the auxiliary accounts: energy (watt-hours), top app by focus time, files touched (counts only, never paths), distinct hosts contacted (salted hashes only, never names), and unlocks. The input tap counts clicks and scrolls alongside keys and mouse travel, and a stale-grant detector flags the macOS signature-change trap ("RE-GRANT — SIGNATURE CHANGED") instead of showing frozen numbers; signing with a local "ByteLife Local" certificate keeps grants stable across rebuilds. The Ledger remains the record layer: the Back Office day dashboard gains auxiliary cards and typing-cadence memos, and receipts book an AUXILIARY section. Sharing and PNG/vector-PDF export are unchanged. The concept brief below and the research under docs/ remain the design ground truth.
